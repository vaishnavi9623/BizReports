        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-100">Filter Invoices</h3>
                </div>

                <!-- Filters Form -->
                <form class="flex flex-wrap items-end gap-2" aria-label="Invoice Filters">
                    <!-- Search Input -->
                    <div class="flex flex-col">
                        <label for="search" class="text-sm text-gray-700 dark:text-gray-300 mb-1">Search by Title or User</label>
                        <input id="search" type="text" placeholder="e.g. Sales Invoice, John Doe"
            ...
            <!-- Invoice Actions and Table -->
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-x-auto p-4">
                <!-- Action Buttons -->
                <div class="flex justify-end mb-4">
                    <a href="{{route('openinvoiceform')}}" class="bg-blue-600 hover:bg-blue-700 text-white text-sm py-2 px-4 rounded-md flex items-center space-x-2 transition duration-200 me-2">
                        <i class="fas fa-plus-circle me-1"></i><span>Create New Invoice</span>
                    </a>
                    <a href="#" class="bg-green-600 hover:bg-green-700 text-white text-sm py-2 px-4 rounded-md flex items-center space-x-2 transition duration-200">
                        <i class="fas fa-file-export me-1"></i><span>Export Invoices</span>
                    </a>
                </div>
                @if(session('success'))
                ...
                <table class="min-w-full text-sm text-gray-700 dark:text-gray-300">
                    <thead class="bg-gray-100 dark:bg-gray-700 text-left text-xs uppercase font-semibold">
                        <tr>
                            <th class="px-6 py-3">Invoice Title</th>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        @forelse($invoicelist as $val)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition duration-200">
                            <td class="px-6 py-4">{{ $val->title }}</td>
                            <td class="px-6 py-4">{{ $val->created_at }}</td>
                            <td class="px-6 py-4">{{ $val->generated_by }}</td>
                            <td class="px-6 py-4">
                                <span class="bg-green-100 text-green-700 text-xs px-3 py-1 rounded-full">{{ $val->status }}</span>
                            </td>
                            <td class="px-6 py-4 flex space-x-3">
                                <a href="#" class="hover:text-blue-600 flex items-center space-x-1 transition duration-150 me-3" title="View Invoice">
                                    <i class="fas fa-eye me-1"></i><span>View</span>
                                </a>
                                <a href="#" class="hover:text-gray-600 flex items-center space-x-1 transition duration-150 me-3" title="Print Invoice">
                                    <i class="fas fa-print me-1"></i><span>Print</span>
                                </a>
                                <a href="#" class="hover:text-indigo-600 flex items-center space-x-1 transition duration-150 me-3" title="Email Invoice">
